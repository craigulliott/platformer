    ...
    module ActiveRecord
      module Associations
        class HasMany < Parsers::AllModels
          for_dsl :has_many do |active_record_class:, foreign_model:, as:, local_column_names:, foreign_column_names:|
            foreign_model_class_name = foreign_model.active_record_class.name

            association_name = as&.to_s&.pluralize&.to_sym || foreign_model_class_name.split("::").last.underscore.pluralize.to_sym

            add_documentation <<~DESCRIPTION
              Add a has many association between this model and #{foreign_model_class_name}.
            DESCRIPTION
            ...
            if local_column_names.any? || foreign_column_names.any?
              ...
              active_record_class.install_method association_name do
                ...
                foreign_model_class_name.consantize.where finder_args
              end

            else
              active_record_class.has_many association_name, class_name: foreign_model_class_name
            end
          end
        end
      end
