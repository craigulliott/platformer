  ...
  module Composers
    module GraphQL
      module Mutations
        ...
        # called, then this composer will generate a `Mutations::Uses::CreateBadges`
        ...
        class Creators < Parsers::Mutations
          ...
          for_dsl :create do |model_reader:, mutation_definition_class:, graphql_type_class:, active_record_class:|
            add_documentation <<~DESCRIPTION
              Create a GraphQL Mutation to create this class which corresponds to this model class.
            DESCRIPTION

            # generate a name like "Mutations::Uses::CreateBadges" from "Uses::Badges"
            ...
            # set the active record model class, so the shared `create` method knows which class to fetch
            ...
            # use the shared create method from the base mutations class
            graphql_mutation_class.alias_method :resolve, :create
