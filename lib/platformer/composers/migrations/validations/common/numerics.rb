        ...
        module Common
          ...
          class Numerics < Parsers::Models::ForFields
            for_numeric_fields do |name:, table:, array:, description:, allow_null:|
              ...
              for_method :validate_greater_than do |value:, description:|
                ...
                check_clause = <<~SQL
                  #{column.name} > #{value}
                SQL
                table.add_validation validation_name, [column.name], check_clause, template: :greater_than, description: final_description
              end

              # if the validate_greater_than_or_equal_to validation was used
              for_method :validate_greater_than_or_equal_to do |value:, description:|
                add_documentation <<~DESCRIPTION
                  Update this models table (`#{column.table.schema.name}'.'#{column.table.name}`)
                  within DynamicMigrations and add a constraint to assert that any values provided
                ...
                check_clause = <<~SQL
                  #{column.name} >= #{value}
                SQL
                table.add_validation validation_name, [column.name], check_clause, template: :greater_than_or_equal_to, description: final_description
              end

              # if the validate_less_than validation was used
              for_method :validate_less_than do |value:, description:|
                add_documentation <<~DESCRIPTION
                  Update this models table (`#{column.table.schema.name}'.'#{column.table.name}`)
                  within DynamicMigrations and add a constraint to assert that any values provided
                ...
                check_clause = <<~SQL
                  #{column.name} < #{value}
                SQL
                table.add_validation validation_name, [column.name], check_clause, template: :less_than, description: final_description
              end

              # if the validate_less_than_or_equal_to validation was used
              for_method :validate_less_than_or_equal_to do |value:, description:|
                add_documentation <<~DESCRIPTION
                  Update this models table (`#{column.table.schema.name}'.'#{column.table.name}`)
                  within DynamicMigrations and add a constraint to assert that any values provided
                ...
                check_clause = <<~SQL
                  #{column.name} <= #{value}
                SQL
                table.add_validation validation_name, [column.name], check_clause, template: :less_than_or_equal_to, description: final_description
              end

              # if the validate_equal_to validation was used
              for_method :validate_equal_to do |value:, description:|
                add_documentation <<~DESCRIPTION
                  Update this models table (`#{column.table.schema.name}'.'#{column.table.name}`)
                  within DynamicMigrations and add a constraint to assert that any values provided
                ...
                check_clause = <<~SQL
                  #{column.name} = #{value}
                SQL
                table.add_validation validation_name, [column.name], check_clause, description: final_description
              end
            end
          end
        end
