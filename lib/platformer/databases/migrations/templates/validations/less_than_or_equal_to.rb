module Platformer
  module Databases
    class Migrations
      module Templates
        module Validations
          class LessThanOrEqualTo < DynamicMigrations::Postgres::Generator::ValidationTemplateBase
            DEFAULT_DESCRIPTION = <<~DESCRIPTION.strip
              This validation asserts that the column value is less than or equal to the provided value
            DESCRIPTION
            ...
            def fragment_arguments
              ...
              options_string = name_and_description_options_string :"#{column_name}_lte", DEFAULT_DESCRIPTION
              {
                ...
                migration: <<~RUBY
                  validate_less_than_or_equal_to :#{validation.table.name}, :#{column_name}, #{value}#{options_string}
                RUBY
              }
            end
          end
