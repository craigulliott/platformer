    ...
    module Models
      module ActionField
        def self.included klass
          klass.define_dsl :action_field do
            ...
            requires :name, :symbol do
              ...
              description <<~DESCRIPTION
                ...
                boolean column named `:unpublished` and a timestamp column named `:published_at`.
              DESCRIPTION
            end
            ...
            requires :action_name, :symbol, kwarg: true do
              ...
              description <<~DESCRIPTION
                ...
                is named `:published` then a suitable action name would be `:publish`. The
                inverse action name will be automatically generated by prepending `un` to the
                action name. So the inverse of `publish` would be `unpublish`.

                Corresponding methods will be automtically created for the ActiveRecord class
                which backs this model. In the example above, the methods would be `publish!`
                and `unpublish!`.
              DESCRIPTION
            end
            ...
            add_method :requires_presence_of do
              requires :attribute_name, :symbol, array: true do
                ...
                description <<~DESCRIPTION
                  ...
                  must contain a value when the model is completing this action.
                  For instance, triggering the `:publish` action may necessitate that the
                  `:published_by` field includes a user.
                DESCRIPTION
              end
            end

            add_method :requires_absence_of do
              requires :attribute_name, :symbol, array: true do
                import_shared :snake_case_name_validator

                description <<~DESCRIPTION
                  Specify an optional array of field names for this model, each of which
                  must be empty when the model is completing this action.
                DESCRIPTION
              end
            end
          end
