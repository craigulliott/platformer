...
RSpec.describe Platformer::Composers::Migrations::Columns::IpAddress do
  ...
  describe "for a User Model which has a ip_address column named my_ip_address that has a default value, allows null and has a description" do
    before(:each) do
      scaffold do
        model_for "Users::User" do
          database :postgres, :primary
    ...
    context "creates the expected columns within the DynamicMigrations table" do
      ...
      it { expect(subject.column(:my_ip_address).description).to eq "This is a description" }
