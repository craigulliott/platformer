...
RSpec.describe Platformer::Composers::Migrations::Columns::PhoneNumber do
  ...
  describe "for a User Model which has a phone_number field that has a default value, allows null and has a comment" do
    before(:each) do
      scaffold do
        model_for "Users::User" do
          database :postgres, :primary
    ...
    context "creates the expected columns within the DynamicMigrations table" do
      context "a column dedicated to the international dialing code" do
        ...
        it { expect(subject.column(:dialing_code).null).to be true }
        ...
        it { expect(subject.column(:dialing_code).default).to eq "+1" }
      end
      ...
      context "a column dedicated to the phone number" do
        ...
        it { expect(subject.column(:phone_number).default).to eq "1234567890" }
      end
    end
  end
end
